...
+MAX_ATTACHMENT_SIZE_MB = 40
+MAX_ATTACHMENT_SIZE_BYTES = MAX_ATTACHMENT_SIZE_MB * 1024 * 1024
+SPLIT_CHECK_INTERVAL = 10  # Check PDF part size every N pages during splitting
+                                 # Default is 10 pages. Override with
+                                 # --split-check-interval command-line option.
+OCR_CHECK_PERCENTAGE = 0.05 # Check 5% of pages for existing text
+OCR_CHECK_MAX_PAGES = 25 # Max number of pages to check for existing text
+OCR_MIN_TEXT_LENGTH_THRESHOLD = 10 # Min text length on a page to consider it OCR'd
+
...
+    parser = argparse.ArgumentParser(description="Outlook transcript exporter")
+    parser.add_argument(
+        "--split-check-interval",
+        type=int,
+        default=SPLIT_CHECK_INTERVAL,
+        help="Check PDF part size every N pages during splitting",
+    )
...
+    args = parse_args()
+    SPLIT_CHECK_INTERVAL = args.split_check_interval
